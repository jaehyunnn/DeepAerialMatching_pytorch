...
class SynthDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if src_image.size()[0]!=self.out_h or src_image.size()[1]!=self.out_w:
            src_image = self.affineTnf(src_image.unsqueeze(0)).squeeze(0)
        if trg_image.size()[0]!=self.out_h or trg_image.size()[1]!=self.out_w:
            trg_image = self.affineTnf(trg_image.unsqueeze(0)).squeeze(0)
            trg_image_jit = self.affineTnf(trg_image_jit.unsqueeze(0)).squeeze(0)
